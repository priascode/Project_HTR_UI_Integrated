...
def distortion_free_resize(image, img_size):
  ...
  #To Check tha amount of padding needed to be done.
  pad_height = h - tf.shape(image)[0]
  pad_width = w - tf.shape(image)[1]

  # only necessary if we want to do same amount of padding on both sides.
  if pad_height % 2 != 0:
    height = pad_height // 2
...
def build_model():
  ...
  # We have two maxpool layers with pool size and strides 2 maxpooling2D(poolsize, stride)
...
# Loading the model with 4 convolutiinal layers
#  performing inference  for model which has loss = 1.0365

custom_objects = {"CTCLayer": CTCLayer}

reconstructed_model = keras.models.load_model("./trained_models/model_50_epoch_c4.h5", custom_objects=custom_objects)
...
def decode_batch_predictions(pred):
    ...
    # Use greedy search. For complex tasks, beam search.
...
# checking results on some test samples.
...
#adding line breaks
def add_line_breaks(text, max_chars_per_line):
    words = text.split()
    lines = []
    current_line = ''
    
    for word in words:
        # Add the word to the current line if adding it doesn't exceed the character limit
    ...
    # Join the lines with newline characters to form the final text with line breaks
    return '\n'.join(lines)

max_chars_per_line = 50
sentence_with_line_breaks = add_line_breaks(sentence, max_chars_per_line)


# storing output in a '.txt' file
f = open("digitized_output.txt", "w")
f.write(sentence_with_line_breaks)
f.close()


# for deleting files after each execution
